...
import {formatViewsNumber, imageExtension, translate_string} from '../../../../utils/helpers/';
...
export function TaxonomyItemMediaCount(props) {
    return (
      ...
      {' ' + props.count} {translate_string(props.count > 1 ? 'medias' : 'media')}
    </span>
    );
}
...
export function MediaItemMetaViews(props) {
    return (
        <span
            className="item-views">{formatViewsNumber(props.views) + ' ' + translate_string(1 >= props.views ? 'view' : 'views')}</span>
    );
}
