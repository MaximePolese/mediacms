...
export class SearchPage extends Page {
    ...
    updateRequestUrl() {
        ...
        if (null !== this.state.resultsCount) {
            if (!validQuery) {
                title = translate_string('No results for') + ' "' + this.state.searchQuery + '"';
            } else {
                if (this.state.searchCategories) {
                    ...
                    if (null === this.state.resultsCount || 0 === this.state.resultsCount) {
                        ...
                        title = this.state.resultsCount + ' ' + translate_string('result' + (1 < this.state.resultsCount ? 's' : '') + ' for') + ' "' + this.state.searchQuery + '"';
                    }
                }
            }
        }
